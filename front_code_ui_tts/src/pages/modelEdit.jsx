      ...
      <div className={styles.warnContentCkeckbox}>
        <label>
          ...
          <div className={styles.icon}>
            {checked ? (
              <MdCheckBox className={styles.iconChecked} />
            ) : (
...
function ModelEdit() {
  ...
  const config = {
    headers: {
      Authorization:
        "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJyZXcxMjEyQG5hdmVyLmNvbSIsImF1dGgiOiJST0xFX1VTRVIiLCJ0eXBlIjoiYWNjZXNzIiwiaWF0IjoxNzAwOTI3OTY4LCJleHAiOjE3MDEyODc5Njh9.ECu6v3-g3dMOMVO3drUTilXnlVL8ghYH0iifvTdAf6A",
    },
    params: {
      modelId: modelId,
    },
  ...
  const [isShare, setIsShare] = useState(
    components.model_check === 1 ? true : false
  );
  const [editName, setEditName] = useState(description);
  //이미지 설정 여부
  const [imgState, setImgState] = useState(
    components.image_url === null ? false : true
  );
  ...
  const overlayIdConfirm = () => {
    ...
    axios
      .put(baseUrl + "/users/models", dataConfig, idConfig)
      .then((response) => {
        console.log(response);
        alert("ID가 재할당 되었습니다.");
        navigate("/model");
    ...
    <div className={styles.modelBackground}>
          ...
          {isEditing ? (
            ...
            <div className={styles.modelNameBox}>
              ...
              <div className={styles.modelImgBox}>
                <button
                  data-tooltip-id="nameedit-tooltip"
          ...
          <div className={styles.modelIdBox}>
            <div className={styles.modelIdInnerBox}>
              ...
              <button
                ...
                data-tooltip-content="copy model id"
      ...
      >
        <div className={styles.warnIdBox}>
          모델 id를 재발급 받으시게 되면 기존에 해당 모델을 등록하여 사용하고
          계시던 모든 계정에서 해당 모델이 사용 불가능하게 되며 새롭게 발급받은
          id로 재등록 해주셔야 합니다.
        </div>
      </OverlayWarning>
    </div>
